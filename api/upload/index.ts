    ...
    if (contentType.startsWith('image/')) {
      try {
        ...
        const base = filename.replace(/\.[^.]+$/, '')
          .replace(/[^a-zA-Z0-9_-]/g, '') || `upload-${Date.now()}`;
        ...
        const fullStored: BlobPutResult = await put(`${base}.jpg`, optimized, {
          access: 'public',
          contentType: 'image/jpeg',
        } as any);

        const thumbStored: BlobPutResult = await put(`${base}.thumb.webp`, thumb, {
          access: 'public',
