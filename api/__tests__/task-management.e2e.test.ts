...
// The handler pipeline runs devAuth + authenticateJWT (requireAuth). Pass both
// through without injecting a user: createMockAuthRequest attaches req.user, and
// the unauthenticated scenarios keep it undefined so the handler's 401 fires.
// (asyncHandler is mocked to a pass-through here, so a thrown auth error would
// escape instead of becoming a clean response.)
