            ...
            {uiState.isEditing ? (
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  {showTaskListLabel && !calendarMode && (
                    (() => {
                      const resolvedGroupId = task.taskListId ?? 'default';
                      const taskList =
                        taskGroups.find(group => group.id === resolvedGroupId) ||
                        (!task.taskListId
                          ? { id: 'default', name: 'Tasks', emoji: '📋', color: '#3b82f6' }
                          : undefined);
                      return taskList ? (
                        <span
                          className={cn(
                        ...
                        >
                          ...
                          <span className="inline-block whitespace-nowrap">{taskList.name}</span>
                        </span>
                      ) : null;
                    })()
                  )}
