...
interface TaskManagementWithOperations {
  ...
  handleCreateTaskGroup: (data: { name: string; description: string; iconId: string; color: string }) => void;
...
// Function overloads
export function useTaskManagement(options: { includeTaskOperations: true }): TaskManagementWithOperations;
export function useTaskManagement(options?: { includeTaskOperations?: false }): TaskManagementWithoutOperations;
export function useTaskManagement(options: UseTaskManagementOptions = {}): TaskManagementWithOperations | TaskManagementWithoutOperations {
  ...
  const handleAddTask = includeTaskOperations ? (title: string, _groupId?: string, smartData?: SmartTaskData) => {
    addTask.mutate({
      title,
  ...
  const handleToggleTask = includeTaskOperations ? (id: string) => {
  ...
  const handleEditTask = includeTaskOperations ? (id: string, title: string) => {
    updateTask.mutate({
      id,
      updates: { title }
    });
  ...
  const handleDeleteTask = includeTaskOperations ? (id: string) => {
    deleteTask.mutate(id);
  } : undefined;

  const handleScheduleTask = includeTaskOperations ? (id: string) => {
    scheduleTask.mutate(id);
  } : undefined;

  const handleRemoveTag = includeTaskOperations ? (taskId: string, tagId: string) => {
    updateTask.mutate({
      id: taskId,
      updates: {
