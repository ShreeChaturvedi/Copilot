    ...
    if (!user) {
      ...
      if (user) {
        ...
        const created = await withTransaction(async (tx) => {
          ...
          // Give every new user a default calendar so the calendar view isn't
          // empty on first load. The design-system blue (#0d97d5) matches the
          // frontend preset; hardcoded so the backend stays free of frontend
          // imports. Only NEW users reach this branch, so existing-user logins
          // never create a second calendar.
          await query(
