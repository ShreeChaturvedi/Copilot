...
describe('JWT Utilities', () => {
  ...
  describe('generateTokenPair', () => {
    ...
    it('embeds the role claim in a fresh access token when provided', async () => {
      const tokenPair = await generateTokenPair(testUserId, testEmail, 'ADMIN');
      const accessDecoded = jwt.decode(tokenPair.accessToken) as JWTPayload;
      expect(accessDecoded.role).toBe('ADMIN');
    });

    it('omits the role claim when none is provided (legacy tokens)', async () => {
      const tokenPair = await generateTokenPair(testUserId, testEmail);
      const accessDecoded = jwt.decode(tokenPair.accessToken) as JWTPayload;
      expect(accessDecoded.role).toBeUndefined();
    });
  });
